...
try {
    ...
    if (-not $ConfigPath) {
        $ConfigPath = Join-Path $ProjectRoot 'config/variables.yml'
    }
    ...
    Write-Host 'Set this value in config/variables.yml as storage.base_vhd_path before running Deploy-VMFleet.ps1.' -ForegroundColor Yellow
}
catch {
